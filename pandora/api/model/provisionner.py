...
class Provisioner(db.Model):
    ...
    state = db.Column(db.String(20), nullable=False,
                      default=config.get('PROVISIONER_UNKNOWN_STATE'))
    ...
    def list_engines(cls, self):
        ...
        if engines is None:
            from pandora.engines import all as engines_availables
    ...
    def get_engine_cls(self):
        """ Return engine class. """
        try:
            module_path = 'pandora.engines'.join(self.engine.split('.'))
    ...
    def save(self, check_status=True, **kwargs):
        with app.app_context():
            ...
            self.verbose_name = getattr(
                self.get_engine_cls(), 'verbose_name', self.engine)
