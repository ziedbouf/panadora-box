...
from pandora.helpers import prefix_to_num
    ...
    def resources_by_node(self):
        ...
        for node_name, pods in nodes:
            if node_name not in out:
                out[node_name] = {'limits': {
                    'cpu': 0, 'memory': 0}, 'requests': {'cpu': 0, 'memory': 0}}
                        ...
                        for resource_policy in ['limit', 'requests']:
                            ...
                            if policy:
                                for resource_type in ['cpu', 'memory']:
                                    ...
                                    if value:
                                        out[node_name][resource_policy][resource_type] += prefix_to_num(
                                            value)
